...
The project has enough evidence for a defensible workshop paper about
...
| --- | --- | --- |
| Workshop short paper | Strong | Compiled manuscript, generated tables, bootstrap intervals, claim audit, qualitative appendix, and reviewer stress test are in place. |
| High-impact top-tier paper | Promising but incomplete | Now has a bounded TAB legal-domain diagnostic, a pattern-only Presidio-style baseline, a negative TAB legal-utility screen, a negative legal-role replacement check, a no-combo ablation, a 50-record GPT-5.5 attacker check, a 20-record blind mini-attacker check with multi-model agreement analysis, a negative 50-record privacy-aware utility screen, a placeholder fallback diagnostic, a fluent rewrite diagnostic, an attribute-level fluent diagnostic, and a no-API text-quality audit. The fluent-family branches fix local surface artifacts or improve no-API specificity but still do not recover public utility, and the text-quality audit suggests phrase-level specificity is not the missing ingredient. Claims still cannot scale beyond a pilot. |
...
| RQ4: utility preservation | Mixed | Synthetic utility facts support a large RB-QIG edge over blanket: +28.3 points [25.2, 31.6]. Synthetic LLM utility gives only +2.6 [0.6, 4.8]. Budget-fixed blind public semantic utility is tied with blanket: -0.4 [-4.2, 3.4]. A 50-record privacy-aware public utility screen is negative: -6.4 [-10.0, -2.8]. Safe generalization removes literal instruction strings but remains negative: -5.6 [-9.2, -1.6] versus blanket. A budgeted placeholder fallback improves over current RB-QIG (+3.2 points privacy-aware utility, CI [-0.8, 7.2]) but remains tied/below blanket (-3.2 points, CI [-7.2, 0.8]) and loses specificity. A context-aware fluent rewrite preserves specificity and fixes local artifacts, but method-labeled privacy-aware utility is still below blanket: -4.8 points [-9.6, 0.4]. A same-text consistency audit finds 31 privacy-aware utility disagreements among 69 exact-identical transformed-text method pairs, so fine-grained public utility rankings are noisy. A corrected hidden-label deduplicated rerun eliminates same-text disagreements but remains negative: balanced minus blanket -4.0 [-7.2, -0.4], fluent minus blanket -5.2 [-8.8, -1.2]. An attribute-level fluent diagnostic improves no-API specificity to 38.1% but worsens hidden-label utility: attr-fluent minus blanket -12.0 [-16.0, -8.0] and attr-fluent minus fluent -6.8 [-11.2, -2.4]. A no-API text-quality audit shows lower surface-artifact scores do not predict better utility: attr-fluent has the lowest artifact score but worst utility, and longer/generalized replacements correlate negatively with utility in the pooled audit. The bounded 20-document TAB legal-task utility screen is also negative/tied: balanced minus blanket -2.0 [-8.0, 4.0], utility-budget minus blanket -2.0 [-7.0, 3.0]. A legal-role TAB variant is worse than blanket on legal-task utility: -6.0 [-14.0, 2.0]. | Scope utility wins to controlled synthetic settings; present public utility as a caveat. |
...
| Low-cost LLM-attacker protocol | Implemented | `src/llm_attack_eval.py`, `src/analyze_attacker_agreement.py`, `results/ratbench_d1_api_100/llm_attacker_metrics_with_llm_direct.csv`, `results/ratbench_d1_blind_backstop_v2_budgetfix_api_100/llm_attacker_metrics.csv`, `results/ratbench_d1_blind_backstop_v2_budgetfix_api_100/llm_gpt55_bootstrap_report.md`, `results/ratbench_d1_blind_backstop_v2_budgetfix_api_100/attacker_agreement_multimodel_report.md` | Stronger-attacker robustness and bounded multi-model agreement are now tested. |
...
| TAB legal benchmark | Partial deterministic screen plus bounded 20-document legal-utility screen done | `results/tab_echr_dev_30/tab_screen_report.md`, `results/tab_echr_dev_30/llm_legal_utility_20_with_b6_bootstrap_report.md`, `results/tab_echr_dev_30_legal_role/legal_role_screen_report.md` | Supports cross-domain residual-risk motivation; legal utility is negative/tied and should not be used as a utility win. |
...
| Attacker strength | Done for bounded robustness | 20-record `gpt-5.4-mini` target-aware smoke, 20-record blind mini check, 50-record GPT-5.5 blind-backstop check, and multi-model agreement report. | GPT-5.5 is harsher but preserves the direct-to-RB-QIG reduction; use it as a caveat rather than a headline win. |

## Artifact Readiness

| Artifact | Status |
| --- | --- |
| Main manuscript | `paper/main.tex` compiles as a COLM-formatted `paper/main.pdf`. |
...
| Claim audit | `paper/CLAIM_AUDIT.md`, currently 17/17 claim groups matched. |
| Reviewer stress test | `paper/REVIEWER_STRESS_TEST.md`. |
| Reproducibility commands | `README.md` and `RESEARCH_STATUS.md`. |
| API budget tracking | `RESEARCH_STATUS.md`, currently about $6.25 fresh-equivalent cached cost. |
...
| --- | --- | --- | --- |
| 1 | Build a constrained sentence-level rewriter or task-aware generalization policy. | The fluent diagnostic shows local phrase rules can fix visible artifacts and preserve specificity, and attr-fluent shows attribute-level budgeting can improve no-API specificity. Neither establishes a public utility win; the hidden-label deduplicated utility reruns remove same-text judge inconsistency while preserving the negative public-utility result. The text-quality audit further suggests that phrase-level surface cleanup and more typed detail are not enough. | Do no utility-judge spending until a no-API candidate beats placeholder on the text-quality audit while preserving RB-QIG specificity. If no such candidate exists, close phrase-level rewriting and move to a sentence-level/task-aware policy. |
| 2 | Expand multi-model attacker agreement only if required by reviewers. | A bounded mini/nano/GPT-5.5 agreement check is now done and shows moderate agreement with stable qualitative ordering. | Do not expand unless the target venue needs tighter agreement intervals. |
| 3 | Tighten the manuscript after final template choice. | The evidence is stronger than the current space-limited narrative can fully express. | Preserve the page limit and do not broaden claims. |
